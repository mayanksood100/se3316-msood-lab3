...
function getSubjects (){
    fetch(`api/courses/${subjectInput.value}`)
    .then((res) => res.json()
    .then((data) => {
...
function getCourseCodes (){
    fetch(`api/courses/${subjectInput.value}/${courseNumberInput.value}`)
    .then((res) => res.json()
    .then((data) => {
        console.log(res.status);
        let subject = data.filter((c) => c.subject == subjectInput.value);
        let catalogNbr = subject.filter(c => c.catalog_nbr==courseNumberInput.value);
...
document.getElementById('search3').addEventListener('click', getCourseComponent);

function getCourseComponent (){
    fetch(`api/courses/${subjectInput.value}/${courseNumberInput.value}/${componentInput.value}`)
    .then((res) => res.json()
    .then((data) => {
        console.log(res.status);
        let subject = data.filter((c) => c.subject == subjectInput.value);
        let catalogNbr = subject.filter(c => c.catalog_nbr==courseNumberInput.value);
        let courseComponent = catalogNbr.filter(c=> c.course_info[0].ssr_component == componentInput.value);
